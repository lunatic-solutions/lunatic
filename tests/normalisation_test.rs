...
fn externref_save_drop() {
    ...
    let table = instance
        ...
        .unwrap();
    let value_externref = table.get(0).unwrap().unwrap_externref().unwrap();
    let value = value_externref.data().downcast_ref::<i32>().unwrap();
    assert_eq!(*value, 0);

    let value_externref = table.get(1).unwrap().unwrap_externref().unwrap();
    let value = value_externref.data().downcast_ref::<i32>().unwrap();
    assert_eq!(*value, 1);

    let value_externref = table.get(2).unwrap().unwrap_externref().unwrap();
    let value = value_externref.data().downcast_ref::<i32>().unwrap();
    assert_eq!(*value, 2);
    ...
    externref_drop.call(&[Val::I32(1)]).unwrap();
    let value_externref = table.get(1).unwrap().unwrap_externref();
    assert!(value_externref.is_none());

    externref_drop.call(&[Val::I32(3)]).unwrap();
    let value_externref = table.get(3).unwrap().unwrap_externref();
    assert!(value_externref.is_none());
