...
use crate::{
    async_map, module::Module, process::ProcessId, state::HashMapId, EnvConfig, Environment,
    Process, Signal,
};
...
impl SignalOverNetwork {
    async fn from(signal: Signal, node: Node) -> Result<Self> {
        match signal {
            Signal::Message(message) => {
                ...
                for resource in message.resources.into_iter() {
                    match resource {
                        ...
                        crate::message::Resource::TcpStream(_) => {
                            return Err(anyhow!("Resource TcpStream can't be sent to another node"))
                        }
                    }
                }

                let msg = MessageOverNetwork {
                    id: message.id,
                ...
                Ok(SignalOverNetwork::Message(msg))
            }
    ...
    async fn into(self, peer: Peer, node: Node) -> Result<Signal> {
        match self {
            SignalOverNetwork::Message(message) => {
                let mut resources = Vec::with_capacity(message.resources.len());
...
mod tests {
    ...
    async fn spawn_linked_remote_process() {
        ...
        let (handle, process) = crate::spawn(|this, mailbox| async move {
            ...
            // Wait on link death
            assert_eq!(mailbox.pop().await.is_signal(), true);
            Ok(())
        });
